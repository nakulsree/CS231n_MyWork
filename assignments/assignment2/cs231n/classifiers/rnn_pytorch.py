...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        h_embed = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == 'rnn':
          h = rnn_forward(h_embed, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
          h = lstm_forward(h_embed, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        current_word = self._start * torch.ones(N, dtype=torch.long)

        for t in range(max_length):
          x_embed = word_embedding_forward(current_word, W_embed)
          h = rnn_step_forward(x_embed, h, Wx, Wh, b)
