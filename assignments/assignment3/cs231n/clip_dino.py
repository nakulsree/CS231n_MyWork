...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    dot = text_features @ image_features.T
    text_norm = torch.linalg.norm(text_features, dim=1, keepdims=True)
    image_norm = torch.linalg.norm(image_features, dim=1, keepdims=True)

    similarity= dot / (text_norm * image_norm.T)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    processed_images = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
    image_features = clip_model.encode_image(processed_images)
    
    similarity = get_similarity_no_loop(text_features, image_features)

    pred_class_indices = torch.argmax(similarity, dim=0)

    pred_classes = [class_texts[i] for i in pred_class_indices]
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        processed_images = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
        image_features = clip_model.encode_image(processed_images)
        self.normalized_image_features = image_features / torch.linalg.norm(image_features, dim=1, keepdims=True)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        ############################################################################
        query_tokens = clip.tokenize(query)
        query_features = self.clip_model.encode_text(query_tokens)
        normalized_query_features = query_features / torch.linalg.norm(query_features, dim=1, keepdims=True)

        similarity = normalized_query_features @ self.normalized_image_features.T
