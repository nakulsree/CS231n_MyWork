...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # memory must be (N, S, D). Features are (N, D). So unsqueeze(1) to make them (N, 1, D).
        memory = self.visual_projection(features).unsqueeze(1)

        # Make it so that each decoder word can only see up to the words up to it
        tgt_mask = torch.tril(torch.ones(T, T))
        
        decoder_out = self.transformer(tgt=embeddings, memory=memory, tgt_mask=tgt_mask)
