...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # pe[:, :S, :] -s take all batches, first S rows, all dimensions
        positional_stamps = self.pe[:, :S, :]

        output = x + positional_stamps
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = (query @ self.query.weight.t()) + self.query.bias
        K = (key @ self.key.weight.t()) + self.key.bias
        V = (value @ self.value.weight.t()) + self.value.bias

        # To make it from (N, T, H, E/H) to (N, H, T, E/H) for matrix multiplication
        Q = Q.reshape(N, S, H, dk).transpose(1, 2)
        K = K.reshape(N, T, H, dk).transpose(1, 2)
        V = V.reshape(N, T, H, dk).transpose(1, 2)


        # To make it from K (N, H, T, E/H) to K (N, H, E/H, T)
        # so that (N, H, T, E/H) * (N, T, E/H, T) = (N, H, T, T)
        ...
        out = weights @ V # (N, H, T, dk)

        # Recombine (N, H, S, dk) -> (N, S, H*dk)
        out = out.transpose(1, 2).reshape(N, S, E)


        output = out @ self.proj.weight.t() + self.proj.bias
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        ############################################################################
        shortcut = tgt
        # attn_mask=None because we can see the full encoder's output
        # before we needed it because the decoder shouldn't be able to see its whole sequence
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        P = self.patch_size
        # From (N, C, H, W) to (N, C, H/P, P, W/P, P)
        # Splitting the height and the width into P H/P and W/P patches
        x = x.reshape(N, C, H // P, P, W // P, P)

        # Permute to group patches together
        # New shape: (N, H/P, W/P, C, P, P)
        x = x.permute(0, 2, 4, 1, 3, 5)

        # Flatten into (N, num_patches, patch_dim)
        # num_patches = (H/P) * (W/P), patch_dim = C * P * P - used for later question
