...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        all_sims_k = torch.stack([sim(z_k, z) for z in out])
        all_sims_k_N = torch.stack([sim(z_k_N, z) for z in out])

        num1 = torch.exp(sim(z_k, z_k_N) / tau)
        num2 = torch.exp(sim(z_k_N, z_k) / tau)

        den1 = torch.sum(torch.exp(all_sims_k / tau)) - torch.exp(sim(z_k, z_k) / tau)
        den2 = torch.sum(torch.exp(all_sims_k_N / tau)) - torch.exp(sim(z_k_N, z_k_N) / tau)


        l1 = -torch.log(num1 / den1)
        l2 = -torch.log(num2 / den2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    pos_pairs = torch.stack([sim(out_left[i], out_right[i]) for i in range(out_left.shape[0])])
    # Turning from 1D vector into column 2D vector    
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    # Inner torch.stack takes the vectors for a specific row and finds the dot product of one vector with all others
    # Outer torch.stack takes the vectors from different rows and stacks them up
    sim_matrix = torch.stack([
        torch.stack([sim(out[i], out[j]) for j in range(out.shape[0])]) 
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim = compute_sim_matrix(out)
    exponential = torch.exp(sim / tau)
    ...
    pos_pair_sim = torch.cat([torch.diag(sim, diagonal=N), torch.diag(sim, diagonal=-N)])
    # Can also do
    # pos_pair = sim_positive_pairs(out_left, out_right)
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(pos_pair_sim / tau)
